...
Sortiranje je torej pogosto in pomembno opravilo zaradi česar se je je skozi čas
...
v definiciji sortiranja imenovano $K$.
...
in berljiva in je namenjena temu, da si jo uporabnik shrani. Ker je predstavljena kot niz
...
\begin{description}
  ...
  \item[število] predstavlja katerokoli možno dolžino polja, ki ga sortiramo. Torej je lahko
...
konfiguracije zanj. Ker ne vemo, katera konfiguracija je optimalna, jo moramo poiskati.

Glavna ideja je, da lahko dva kompozitna sortirna algoritma lahko primerjamo po času 
...
\noindent Ustvarjanje kandidatov poteka po sledečem algoritmu: \\
za vsak sortirni algoritem $s$: \\
\begin{quote} %% ugly
  \vspace{-4.8ex}
kandidat je trenutni najboljši, ki pa namesto do $\infty$ z zadnjim sortirnim algoritmom
sortira le do prejšnjega $n$, potem pa naprej s $s$.
\end{quote}
...
pri velikih $n$, se ga lahko pospeši. Določene $n$ izpustimo in to tako, da jih pri krajših
...
\begin{enumerate}
  \item Ustvarimo prazno konfiguracijo, v kateri bomo shranjevali najboljšo najdeno
    konfiguracijo do sedaj. Imenujmo jo $naj$. Nastavimo trenutno dolžino polja, na katerem testiramo
    sortirne algoritme za poljubno začetno vrednost. Imenujmo to dolžino $n$. Pri svoji
    implementaciji sem za to vrednost izbral 4, ker je ta vrednost dajala najboljše
    rezultate. Prav tako nastavimo trenutno razdaljo med elementi na $1$. To prikazuje
    algoritem \ref{algo:conf} na vrsticah \ref{line:confinit} -- \ref{line:endconflimit}.
  \item Če je $n$ večji ali enak $M$, meji učenja, potem zaključi, saj smo se že našli
    optimalno konfiguracijo do $M$. Na vrstici \ref{line:beginwhile} je to zapisano kot
    pogoj v \emph{while} zanki.
  \item Najdi najboljšo konfiguracijo pri trenutnem $n$ in si to vrednost shrani v
    spremenljivko $nova$. To naredi funkcija \textsc{izberi\_najboljšo\_pri\_n}, na
    vrstici \ref{line:bestatn}. 
  \item Če je najboljša konfiguracija $naj$ prazna, je torej najboljša konfiguracija kar
    $nova$. Nastaviti pa moramo tudi vrednost, do katere je najboljši ta algoritem, ki ga
    konfiguracija opisuje, in sicer je to ravno $n$, naš trenutni napredek proti meji
    učenja. V algoritmu \ref{algo:conf} je to opisano v \emph{if} bloku, na vrsticah od
    \ref{line:ifconf} do \ref{line:endifconf}.
  \item Če pa je algoritem, ki ga kot zadnjega uporablja $nova$, enak tistemu, ki ga kot
    zadnjega uporablja $naj$, potem moramo samo spremeniti mejo uporabe algoritma pri
    $naj$, povečati jo moramo na trenutni $n$. Ta del predstavlja \emph{else if} blok na
    vrsticah \ref{line:elseifconf} in \ref{line:endelseifconf}. 
  \item Če $nova$ zadnji algoritem, ki ga uporablja nova, ni enak zadnjemu, ki ga
    uporablja $naj$ potem moramo ugotoviti kje je meja med tem, ki ga uporablja $nova$ in
    tem, ki ga uporablja $naj$. To naredi funkcija \textsc{presečišče}. Ker pa morata biti
    obe konfiguraciji, ki ju sprejme ta funkcija smiselni moramo pred nastaviti zadnjo
    mejo konfiguracije $naj$ na $-1$. Vrednost, ki jo
    vrne ta funkcija potem nastavimo za limit uporabe $naj$. Nato pa naj dodamo še en
    vnos. Dodati moramo, da naj se nov sortirni algoritem uporablja do trenutnega $n$.
    Zgornji del algoritma je prikazan na vrsticah \ref{line:elseconf} -- \ref{line:endelseconf}.
  \item Nato $n$ povečamo za $razdaljo$ in $razdaljo$ za $pospe\check{s}ek$ (vrstici
    \ref{line:n++} in \ref{line:razdalja++}). Tako
    zagotovimo, da se bodo razmaki med testiranimi dolžinami povečevali s kvadratom
    števila narejenih iteracij. S tem se sicer zmanjša natančnost na večjih poljih, vendar
    se algoritem zato precej pospeši. Potem ponovimo vse korake od \ref{line:beginwhile}
    naprej. To je prikazano kot \emph{while} zanka na vrsticah od \ref{line:beginwhile} do
    \ref{line:endwhile}.
\end{enumerate}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State \label{line:endconflimit}$razdalja \gets $ izbrana začetna vrednost
    \While{$n < M$} \label{line:beginwhile}
        \State \label{line:bestatn} $nova \gets $\textsc{izberi\_najboljšo\_pri\_n}$(n,
        $\textsc{naredi\_možnosti}$(naj), cmp)$
      \If{$naj = ""$} \label{line:ifconf}
        \State $naj \gets nova$
        \State $naj.$\textsc{nastavi\_zadnjo\_mejo}$(n)$ \label{line:endifconf}
      \ElsIf{$naj.last.algo = nova.last.algo$} \label{line:elseifconf}
        \State $naj.$\textsc{nastavi\_zadnjo\_mejo}$(n)$ \label{line:endelseifconf}
      \Else \label{line:elseconf}
        \State $naj.$\textsc{nastavi\_zadnjo\_mejo}$(-1)$
        \State $pres \gets $\textsc{presečišče}$(n-(razdalja-pospe\check{s}ek), n, naj, nova)$
        \State $naj.$\textsc{nastavi\_zadnjo\_mejo}$(pres)$
        \State $naj \gets naj\ + $``$:nova.last.algo \lra n;$''
        \EndIf \label{line:endelseconf}
      \State \label{line:n++} $n \gets n + razdalja$
      \State \label{line:razdalja++} $razdalja \gets razdalja + pospe\check{s}ek$
    \EndWhile \label{line:endwhile}
  \end{algorithmic}
\end{algorithm}

Funkcija \textsc{naredi\_možnosti} je funkcija, ki vrne množico možnih konfiguracij z dano
osnovo. To naredi tako, da osnovi vsakič doda eno izmed osnovnih konfiguracij in vrne vse
možnosti. Tako ustvari vse možne najboljše konfiguracije z dano osnovo.

Funkcija \textsc{presečišče} je funkcija, ki vrne mejno točko med dvema sortirnima
algoritmoma, opisani v poglavju %% \ref{chapter:bisekcija}.
V psevdokodi je funkcija \textsc{presečišče} definirana kot algoritem \ref{algo:intersection}.
\begin{enumerate}
  \item Na vrsticah \ref{line:findlow} in \ref{line:findhigh} algoritma
  ...
  \item Če sta spodaj in zgoraj enaka, presečišča ni na intervalu $[m, M]$ in zato
    vrnemo ali zgornjo ali spodnjo mejo, tisto, ki je bližje pravemu presečišču. To ne
    naloga \emph{if} stavka na vrsticah \ref{line:interif} do \ref{line:endinterif}.
  \item \label{line:intervalwidthcheck}Če je razlika med $m$ in $M$ manjša od 1, potem je $m$ enak $M$ in je tam tudi
  ...
  \item Preverimo, kateri algoritem je boljši pri srednji vrednosti $\frac{m+M}{2}$
  ...
  \item Ponovimo korake od \ref{line:intervalwidthcheck} naprej. To ponavljanje je
...
\begin{enumerate}
  ...
  \item Pripravimo si želeno število polj. To naredi na vrstici
    ...
    kot sicer.
  \item Zapomnimo si trenuten čas (vrstica \ref{line:getstarttime}), nato uredimo vsa 
...
\section{Rezultati}
Moj kompozitni sortirni algoritem sem primerjal z vsakim posameznim algoritmom izmed
tistih, ki so bili opisani v teoretičnem uvodu v poglavju \ref{chapter:teoreticni}.



\begin{table}
  \centering
  \caption{Rezultati za tip \textbf{int}.}
  \label{tab:rez:int} \vspace{1ex}
  \begin{tabular}{|l|r|r|r|}
    \hline
    algoritem $\backslash$ št. elementov & 10   & 100         &        1.000  \\ \hline 
    urejanje z vstavljanjem        &   27,48 ns &  3899,96 ns &  259741,30 ns \\ \hline
    urejanje z izbiranjem          &  170,85 ns & 11163,06 ns & 1173386,60 ns \\ \hline
    hitro urejanje                 &  352,55 ns &  6410,38 ns &   80817,60 ns \\ \hline
    urejanje s kopico              &   85,92 ns &  7299,30 ns &   93082,60 ns \\ \hline
    urejanje z zlivanjem           & 4532,12 ns & 57229,09 ns &  582053,60 ns \\ \hline
    vgrajeni algoritem v C++       &   40,28 ns &  3532,53 ns &   59830,60 ns \\ \hline
    moje urejanje                  &   43,01 ns &  3290,16 ns &   58692,90 ns \\ \hline
  \end{tabular}
\end{table}
