...
V našem primeru bomo iskali presečišče dveh sortirnih algoritmov. 
Tako za funkcijo $f$ lahko vzamemo kar razliko v času urejanja med tema algoritmoma. Ko sta razliki nasprotno predznačeni,
...
\begin{enumerate}[1.)]
  ...
  \item zapišemo najboljšega v konfiguracijo
...
algoritmom sortira le do prejšnjega $n$, potem pa naprej s $s$. Tako ustvarimo množico
...
\begin{enumerate}
  ...
  \item \textbf{Preverimo pozicijo presečišča.} Če sta spodaj in zgoraj enaka, presečišča ni na intervalu $[m, M]$ in zato
  ...
  \item \textbf{Preverimo, če smo našli presečišče.} \label{line:intervalwidthcheck}Če je razlika med $m$ in $M$ manjša od 1, potem je $m$ enak $M$ in je tam tudi
  ...
  \item \textbf{Najdemo sredinsko vrednost.} Preverimo, kateri algoritem je boljši pri srednji vrednosti $\frac{m+M}{2}$
...
To poglavje je namenjeno predvsem predstavitvi implementacije algoritma opisanega v
poglavju \ref{chapter:optimalconf}. Poglavje je namenjeno tudi opisu uporabe knjižnice, ki
...
\begin{itemize}
  ...
  \item Parameter $limit$ pomeni število elementov, ki jih lahko hranimo v pomnilniku.
    Večje ko je to število, hitrejši bo program in bolj natančne bodo meritve. Mora biti najmanj $M$. Program bo v
  ...
  \item Parameter $acceleration$ pomeni pospešek povečevanja dolžine polj, ki jih bo
    program sortiral. Razmik med dolžinami se torej vsako iteracijo poveča za
    $acceleration$. Nujno mora biti pozitiven ali enak $0$. Če je enak nič, potem se bo
    algoritem obnašal kot osnovni algoritem za iskanje optimalne konfiguracije opisan kot
...
primerjanje njihove uspešnosti, zato se mu velja podrobneje posvetit.
...
tako, da uredimo več polj zaporedoma, izmerimo skupni čas urejanja, iz katerega nato
...
\begin{enumerate}
  ...
  \item \textbf{Ugotovimo, koliko polj si lahko pripravimo.} \label{time:start} Ker pa
    polja, ki si jih bomo pripravili za testiranje lahko zavzemajo precej veliko količino
  ...
  \item \textbf{Ponovimo do želenega števila iteracij.} Če je napredek večji ali enak želenemu številu ponovitev, končaj. 
...
tistih, ki so bili opisani v teoretičnem uvodu v poglavju \ref{chapter:teoreticni}. C++ algoritem
je algoritem, ki je že vgrajen v C++, v knjižnici \emph{algoritm}. 
Vse algoritme sem testiral na štirih različnih tipih podatkov: 
\emph{int}, \emph{string}, \emph{huge} in \emph{slow}, ki so oisani v nadaljevanju.
...
\begin{table}[h]
  % \centering
  \caption{Rezultati za tip \emph{int}.}
  ...
  \begin{tabular}{l!{\vrule width 1pt}r|r|r|r|r}
    ...
    moje urejanje        &   169.79 &  3,61435 &   58,69  &    749,768  &      9,35 \\ 
  \end{tabular}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabular}{l!{\vrule width 1pt}r|r|r|r|r}
    \bf št. elementov     & \bf 10 el.& \bf 100 el.& \bf 1.000 el. & \bf 10.000 el. & \bf 100.000 el. \\ 
    \bf št. iteracij      & \bf 100.000 it. & \bf 10.000 it. & \bf 1.000 it. & \bf 10 it. &  \bf 10 it.\\ \hline
    ...
    moje urejanje         & 1,55 &  35,78 &    644,23 &    8.966,94 & \\ 
  \end{tabular}
\end{table}
...
\[ :i \lra 8;:s \lra 68;:q \lra 100;:s \lra 148;:q \lra 189;:s \lra 287;:q \lra -1; \edot \]
...
\begin{table}[h]
  ...
  \begin{tabular}{l!{\vrule width 1pt}r|r|r|r}
    ...
    moje urejanje      &  5,08 &  77,41 &  1.215,61 &     21.840,79 \\ 
  \end{tabular}
\end{table}
...
\[ :i \lra 5;:m \lra 1553;:q \lra 1733;:m \lra 3280; \edot \]
...
\begin{table}[h]
  ...
  \begin{tabular}{l!{\vrule width 1pt}r|r|r|r}
    ...
    moje urejanje     & 16,05 &   382,914 &   5.973,63 &    83.143,84  \\ 
  \end{tabular}
\end{table}

\section{Ugotovitve in razprava}
Uvodek bo še napisan.
...
z vstavljanjem sta boljša od vseh ostalih sortirnih algoritmov, moj kompozitni algoritem in C++-ov 
algoritem, ki je prav tako kompoziten, sta za spoznanje slabša. Pri mojem algoritmu je to posledica
iskanja po konfiguraciji, česar navadnemu urejanju z vstavljanjem ni potrebno delati. Sicer 
se moj algoritem obnaša popolnoma enako kot urejanje z vstavljanjem, saj vidimo, da se polja dolžine 
...
na prvem mestu med nekompozitnimi algoritmi, medtem ko se pri mojem algoritmu že kaže njegova
kompozitna narava, saj pri poljih dolžine 100 že uporablja hitro urejanje, ki polja razdeli na 
manjša podpolja, ki se jih potem, ko je njihova dolžina manjša ali enaka 67 uredi z vstavljanjem.
...
Kompozitna algoritma zdaj že močno pokažeta svoje prednosti, moj algoritem se obnaša popolnoma 
enako kot pri 100 elementih.
...
Pri sto tisoč elementih se spremeni le prvo mesto -- C++-ov algoritem prehiti moj kompozitni 
algoritem. C++-ov algoritem ima boljšo implementacijo osnovnih algoritmov in je tudi
prilagojen na že vgrajene tipe v C++. Poleg tega moj algoritem ni poiskal optimalne 
konfiguracije do sto tisoč elementov, temveč le do deset tisoč. Pri ostalih algoritmih se 
...
\begin{thebibliography}{99}
  ...
  \bibitem[PLE10]{bib:ple10} {Plestenjak, Bor. 2010. \emph{Uvod v numerične metode} (v pripravi)[online]. [Citirano 27. feb. 2011; 15:13].
